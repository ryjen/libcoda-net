...
namespace coda {
  namespace net {
    ...
    bool jester::set_method(const std::string &value) {
      for (int i = 0;
           i < (sizeof(http::method_names) / sizeof(http::method_names[0]));
           i++) {
    ...
    bool jester::is_interactive() const { return interactive_; }

    jester &jester::append_data(const std::string &key,
    ...
    jester &jester::append_header(const std::string &key,
                                  const std::string &value) {
      headers_[key] = value;
      return *this;
    }
    ...
    http::method jester::method() const { return method_; }

    uri jester::uri() const { return uri_; }

    void jester::syntax(const char *bin) const {
      printf("Syntax: %s\n\n", bin);
      puts("\t[path]\trest path to test.  if a uri option has been specified "
           "at least once, it will append to "
           "its value.");
      puts("\nOptions:");
    ...
    int jester::parse_options(int argc, char *argv[]) {
      ...
      while ((opt = getopt_long(argc, argv, "u:m:d:ih:", longopts, NULL)) !=
             -1) {
        switch (opt) {
        case 'u':
          set_uri(optarg);
    ...
    bool jester::validate() const { return uri_.is_valid(); }
