    ...
    namespace net
    {
        namespace http
        {
            ...
            transfer::transfer(const transfer &other)
                : content_(other.content_), headers_(other.headers_), version_(other.version_)
            {
            }
            ...
            string transfer::header(const string &key)
            {
                return headers_[key];
            }
            ...
            const map<string, string> transfer::headers() const
            {
                return headers_;
            }

            string transfer::content() const
            {
                return content_;
            }

            string transfer::version() const
            {
                return version_;
            }

            void transfer::set_version(const string &value)
            {
                version_ = value;
            }
            ...
            response::response(const string &fullResponse) : value_(fullResponse), code_(0)
            {
                parse();
            }

            response::response(const response &other) : transfer(other), value_(other.value_), code_(other.code_)
            {
            }

            response::response(response &&other)
                : transfer(std::move(other)), value_(std::move(other.value_)), code_(std::move(other.code_))
            {
            }
            ...
            string response::full_response() const
            {
                return value_;
            }
            ...
            void response::parse()
            {
                ...
                if (sscanf(line.c_str(), http::RESPONSE_PREAMBLE, version, &code_, buf) != 3) {
                    content_ = value_;
                    return;
                }
            ...
            client::client(const client &other)
                : transfer(other), uri_(other.uri_), response_(other.response_), timeout_(other.timeout_)
            {
            }
            ...
            client &client::add_headers(const std::map<std::string, std::string> &value)
            {
                headers_.insert(value.begin(), value.end());
                return *this;
            }

            void client::remove_header(const string &key)
            {
                headers_.erase(key);
            }
            ...
            client &client::request(http::method method, const std::string &path, const client::callback &callback)
            {
                if (!impl_) {
            ...
            client &client::get(const std::string &path, const client::callback &callback)
            {
            ...
            client &client::post(const std::string &path, const client::callback &callback)
            {
            ...
            client &client::put(const std::string &path, const client::callback &callback)
            {
            ...
            client &client::de1ete(const std::string &path, const client::callback &callback)
            {
