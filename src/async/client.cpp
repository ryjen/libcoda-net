...
namespace coda {
  namespace net {
    namespace async {
      default_client::default_client(SOCKET sock, const sockaddr_storage &addr)
          : buffered_socket(sock, addr) {}

      default_client::default_client(const std::string &host, const int port)
      ...
      default_client::default_client(default_client &&other)
          : buffered_socket(std::move(other)),
      ...
      void default_client::on_connect() {
        backgroundThread_ =
            std::make_shared<std::thread>(&default_client::run, this);
      }
