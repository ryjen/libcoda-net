...
namespace rj
{
    namespace net
    {
        ...
        http_client::http_client(const rj::net::uri &uri) : uri_(uri), timeout_(http::DEFAULT_HTTP_TIMEOUT)
        {
            add_header(http::HEADER_USER_AGENT, THIS_USER_AGENT);
        }

        http_client::http_client(const std::string &uri) : http_client(rj::net::uri(uri, http::PROTOCOL))
        {
        }
        ...
        http_client::http_client(http_client &&other)
            : http_transfer(std::move(other)), uri_(std::move(other.uri_)), response_(std::move(other.response_)), timeout_(other.timeout_)
        {
        }
        ...
        rj::net::uri http_client::uri() const
        {
            return uri_;
        }
