  ...
  namespace net {
    namespace encoding {
      namespace input {
        ...
        struct is_type
            : std::integral_constant<bool,
                                     std::is_same<T, input::map>::value ||
                                         std::is_same<T, input::str>::value> {};
      } // namespace input

      // a base encoder class, T is the type after encoding (and thus the type
      // being decoded from)
      template <typename T> class encoder {
        ...
        // encode a input
        template <typename I,
                  typename = std::enable_if<encoding::input::is_type<I>::value>>
        ...
        protected:
        virtual void encode(const encoding::input::map &from,
                            encoded_type &to) const = 0;
        virtual void encode(const encoding::input::str &from,
                            encoded_type &to) const = 0;
        virtual void decode(const encoded_type &from,
                            encoding::input::map &to) const = 0;
        virtual void decode(const encoded_type &from,
                            encoding::input::str &to) const = 0;
      };
